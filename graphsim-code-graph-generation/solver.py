...
from utils import *
from models import Generator, Discriminator, WLSDiscriminator, WLSDiscriminatorLinear
from data.sparse_molecular_dataset import SparseMolecularDataset

log_filename = 'log_losses.txt'
...
class Solver(object):
    ...
    def build_model(self):
        ...
        elif self.config.discriminator == 'WLS':
            self.D = WLSDiscriminatorLinear(self.data.atom_num_types, self.data.bond_num_types, 4, 50)
            self.V = WLSDiscriminatorLinear(self.data.atom_num_types, self.data.bond_num_types, 4, 50)
        else:
            raise ValueError('Discriminator shall be either "default" or "WLS"')
        ...
        print(f'Generator "default", parameters : {sum([p.numel() for p in self.G.parameters()])}')
        print(f'Discriminator "{self.config.discriminator}", paramters :'
                f'{sum([p.numel() for p in self.D.parameters()])}')
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i+1) % self.n_critic == 0:
                ...
                self.reset_grad()
                g_loss.backward()
                loss['G/grad_norm'] = np.mean([p.grad.norm(2).item() for p in self.G.parameters() if p.grad is not None])
            ...
            if (i+1) % self.log_step == 0:
                ...
                loss['Num_valid'] = sum([1 for i in mols if i])
                loss['Total_gen'] = len(mols)

                loss['Discrimiantor'] = self.config.discriminator
                loss['GPU'] = self.config.gpu
                loss['step'] = i+1

                if not os.path.isfile(log_filename):
                    with open(log_filename, 'w') as of:
                        print('\t'.join(map(str, loss.keys())), file=of)

                with open(log_filename, 'a') as of:
                    print('\t'.join(map(lambda x:f'{x:.3f}' if isinstance(x, float) else str(x),
                            [v for k,v in loss.items()])), file=of)

                for k,v in list(loss.items())[:6]:
                    print(f'\t{k}:{v:.3f}', end='')
                print(f'\t{loss["Num_valid"]}', end='')
                print(f'\t{loss["Total_gen"]}', end='')
                print(f'\titer:{i+1}')
